...
function initDatatable(isOrdered){
    $('.datatable').dataTable( {
        ...
        "sAjaxSource": isOrdered?"../ServletAction?Action=showSalaryTable&isOrdered=true":"../ServletAction?Action=showSalaryTable&isOrdered=false"
    });
...
function onStatisticRecord(){
    window.location.href="templates/admin/statistic_table.jsp";
}
...
function submitAddRecord(){
    ...
    $.post(url,message,function (json) {
            if (json.ok === 200) {
                window.location.href="salary_table.html";
                alert("添加成功！");
            } else {
                // 否则登录失败
                alert("添加失败！");
...
function submitQueryRecord(){
    $('.datatable').dataTable( {
        ...
        "fnDrawCallback": function(){$(".checkboxes").uniform();$(".group-checkable").uniform();},
        //"sAjaxSource": "get_record.jsp"
        "sAjaxSource": "../ServletAction?Action=querySalaryRecord&employee_name="+$("#QuerySalaryRecord #employee_name").val()+"&salary_month="+$("#QuerySalaryRecord #salary_month").val(),
    });
    $('.datatable').find('.group-checkable').change(function () {
        var set = jQuery(this).attr("data-set");
        var checked = jQuery(this).is(":checked");
